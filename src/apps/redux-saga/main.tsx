...
import { cacheApi } from './redux/api/index.ts';
...
const init = async () => {
  ...
  if (token) {
    const getProfileResponse = await store.dispatch(cacheApi.endpoints.getProfile.initiate());
    providersProps.profile.defaultProfile = getProfileResponse.data?.profile;
    providersProps.session.defaultSession = !!getProfileResponse.data;
  }
